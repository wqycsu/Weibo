...
public class TextViewWithLink extends TextView {
	 ...
	 * 是否消耗了非URL点击的事件
	 */
	boolean notConsumeNonUrlClicks = true;
	/**
	 * 链接是否被点中
	 */
	boolean linkHit;
	...
	public TextViewWithLink(Context context, AttributeSet attrs, int defStyle) {
		...
		public boolean onTouchEvent(TextView widget, Spannable buffer, MotionEvent event) {
			...
			if (action == MotionEvent.ACTION_UP ||
				...
				if (link.length != 0) {
					...
					if (widget instanceof TextViewWithLink){
						((TextViewWithLink) widget).linkHit = true;
					}
					return true;
				} else {
